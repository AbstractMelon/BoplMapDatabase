...
function extendRef(ref2, extend, { enumerable = false, unwrap = true } = {}) {
	if (!isVue3 && !version.startsWith('2.7.')) {
		if (true)
			throw new Error(
				'[VueUse] extendRef only works in Vue 2.7 or above.'
			);
		return;
	}
...
function useBluetooth(options) {
	...
	const isSupported = useSupported(
		() => navigator && 'bluetooth' in navigator
	);
...
function useBreakpoints(breakpoints, options = {}) {
	...
	return Object.assign(shortcutMethods, {
		...
		between(a, b) {
			return useMediaQuery(
				() =>
					`(min-width: ${getValue2(a)}) and (max-width: ${getValue2(
						b,
						-0.1
					)})`,
				options
		...
		active() {
			const bps = current();
			return computed(() =>
				bps.value.length === 0 ? '' : bps.value.at(-1)
			);
		},
	});
}
...
function usePermission(permissionDesc, options = {}) {
	...
	const query = createSingletonPromise(async () => {
		...
		if (!permissionStatus.value) {
			try {
				permissionStatus.value = await navigator.permissions.query(
					desc
				);
			} catch (e) {
	...
	if (window2) {
		let observe2 = function () {
			...
			cleanup2 = function () {
				media == null
					? void 0
					: media.removeEventListener('change', observe2);
			};
		let media;
		observe2();
...
function useDisplayMedia(options = {}) {
	...
	const isSupported = useSupported(() => {
		var _a2;
		return (_a2 = navigator == null ? void 0 : navigator.mediaDevices) ==
			null
			? void 0
			: _a2.getDisplayMedia;
	});
	...
	async function _start() {
		...
		(_a2 = stream.value) == null
			? void 0
			: _a2.getTracks().forEach((t) => t.addEventListener('ended', stop));
		return stream.value;
	}
	...
	if (isClient) {
		...
		const checkValidity = (event) => {
			...
			const multipleFilesValid =
				multiple ||
				items.filter((item) => item.kind === 'file').length <= 1;
		...
		const handleDragEvent = (event, eventType) => {
			...
			switch (eventType) {
				case 'enter':
					...
					(_a2 = _options.onEnter) == null
						? void 0
						: _a2.call(_options, null, event);
					break;
				case 'over':
					(_b2 = _options.onOver) == null
						? void 0
						: _b2.call(_options, null, event);
					break;
				case 'leave':
					counter -= 1;
					if (counter === 0) isOverDropZone.value = false;
					(_c = _options.onLeave) == null
						? void 0
				...
				case 'drop':
					...
					if (isValid) {
						...
						(_d = _options.onDrop) == null
							? void 0
							: _d.call(_options, currentFiles, event);
					}
					break;
			}
		};
	...
	const { stop: stop1 } = useResizeObserver(
		...
		([entry]) => {
			...
			if (window2 && isSVG.value) {
				...
				if (boxSize) {
					const formatBoxSize = Array.isArray(boxSize)
						? boxSize
						: [boxSize];
...
function useMousePressed(options = {}) {
	...
	useEventListener(window2, 'mouseup', onReleased, {
		passive: true,
		capture,
	});
	if (drag) {
		...
		useEventListener(window2, 'drop', onReleased, {
			passive: true,
			capture,
		});
...
function useNavigatorLanguage(options = {}) {
	...
	const isSupported = useSupported(
		() => navigator && 'language' in navigator
	);
...
function useTextareaAutosize(options) {
	...
	const styleProp =
		(_a = options == null ? void 0 : options.styleProp) != null
			? _a
			: 'height';
		...
		if (isVue2) {
			const modelOptions =
				(_e =
					...
					: _e.model;
			key =
				(modelOptions == null ? void 0 : modelOptions.value) || 'value';
			if (!eventName)
				event =
					(modelOptions == null ? void 0 : modelOptions.event) ||
					'input';
		} else {
			key = 'modelValue';
		}
...
function useWakeLock(options = {}) {
	...
	const isSupported = useSupported(
		() => navigator && 'wakeLock' in navigator
	);
	...
	async function forceRequest(type) {
		...
		sentinel.value = isSupported.value
			? await navigator.wakeLock.request(type)
			: null;
	}
...
vitepress/lib/vue-demi.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-JPR6NOF3.js.map
