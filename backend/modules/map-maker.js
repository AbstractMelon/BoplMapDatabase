...
router.post('/upload', isAdmin, upload.fields([
    ...
    if (!versionId || !changelog) {
        return res.status(400).json({ message: 'Missing versionId or changelog' });
    }
    ...
    if (req.files.zipFile1 && req.files.zipFile1[0]) {
        const destPath = path.join(versionDir, `Map Maker Windows v${versionId}.zip`);
        fs.renameSync(req.files.zipFile1[0].path, destPath);
    }

    if (req.files.zipFile2 && req.files.zipFile2[0]) {
        const destPath = path.join(versionDir, `Map Maker Linux v${versionId}.zip`);
    ...
    res.status(201).json({ message: 'Files uploaded successfully', versionData });
});
